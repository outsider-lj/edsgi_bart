...
print(project_dir)
eval_file = xlrd.open_workbook(project_dir.joinpath("edsgi/bart-base/edsgi/edsgi_top5.xls"))
sheet = eval_file.sheet_by_index(0)
sents = []
length=[]
for i in range(1,sheet.nrows):
    ref= sheet.cell_value(i,6).lower().strip()
    gen=sheet.cell_value(i,7).lower().strip()
    # if len(tk) < 2:
    #     continue
    # l = [item for item in map(string.strip, re.split("(\W+)", gen)) if len(item) > 0]
    pred_tokens = nltk.word_tokenize(gen)#re.findall(r"[\w']+|[.,!?;]",gen)#re.split(r'([,.?:！])', gen.strip())#gen.strip().split(r'[，。?:！\s]') gen.strip().split(" ")#
    gold_tokens =nltk.word_tokenize(ref)#re.findall(r"[\w']+|[.,!?;]",ref)#ref.strip().split(r'[，。?:！\s]')
    sents.append([pred_tokens, gold_tokens])
    length.append(len(pred_tokens))
# calc f1
# f1 = calc_f1(sents)
# calc bleu
bleu1, bleu2,bleu3,bleu4,avg = calc_bleu(sents)
# calc distinct
...
output_str = "BLEU1: %.5f%%\n" % bleu1
output_str += "BLEU2: %.5f%%\n" % bleu2
print("BLEU3",bleu3,"BLEU4",bleu4,"AVG",avg)
output_str += "DISTINCT1: %.5f%%\n" % distinct1
output_str += "DISTINCT2: %.5f%%\n" % distinct2
