...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    def get_output_embeddings_first(self) -> nn.Module:
        """
        Returns the model's output embeddings.
    ...
    def get_output_embeddings_second(self) -> nn.Module:
        """
        Returns the model's output embeddings.
        ...
        """
        return None
    def get_convert(self) -> nn.Module:
        """
        Returns the model's output embeddings.
    ...
    def tie_weights(self):
        ...
        the weights instead.
        """
        output_embeddings_first = self.get_output_embeddings_first()
        output_embeddings_second = self.get_output_embeddings_second()
        output_embeddings = self.get_output_embeddings()
        convert=self.get_convert()

        if convert is not None:
            w=self.get_input_embeddings().weight.clone()
            w=torch.matmul(w.transpose(0,1),w)
            convert.weight = nn.Parameter(w.clone())
        if output_embeddings_first is not None and self.config.tie_word_embeddings:
            self._tie_or_clone_weights(output_embeddings_first, self.get_input_embeddings())
        if output_embeddings_second is not None and self.config.tie_word_embeddings:
            self._tie_or_clone_weights(output_embeddings_second, self.get_input_embeddings())
    ...
    # def load_missing_state_dict(self,state_dict,missing_key):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        if state_dict is None and not from_tf:
            try:
                state_dict = torch.load(resolved_archive_file, map_location="cpu")#预训练模型的全部参数权重
            except Exception:
                raise OSError(
        ...
        if from_tf:
            ...
            def load(module: nn.Module,one_state_dict,metadata, prefix=""):
                ...
                for name, child in module._modules.items():#循环获取model中所需要的的参数
                    if child is not None:
                        load(child, one_state_dict,metadata,prefix + name + ".")
            ...
            load(model_to_load, state_dict, metadata, prefix=start_prefix)
            # load(model_to_load.base_model.decoder_final,Decoder_state_dict,decoder_metadata)
            # load(model_to_load.lm_head_first,)
            # 分别loaddecoder1与2
            #mkhbart
            # if config.is_train==True:
            #     inject_keys = []
            #     load(model_to_load.base_model.shared, state_dict, metadata, prefix=cls.base_model_prefix + ".shared.")
            #     load(model_to_load.base_model.encoder, state_dict, metadata, prefix=cls.base_model_prefix + ".encoder.")
            #     load(model_to_load.base_model.decoder, state_dict, metadata, prefix=cls.base_model_prefix + ".decoder.")
            #     load(model_to_load.base_model.decoder1,state_dict,metadata,prefix=cls.base_model_prefix + ".decoder.")
            #     load(model_to_load.base_model.decoder2, state_dict, metadata,
            #          prefix=cls.base_model_prefix + ".decoder.")
            #     load(model_to_load.base_model.emo_layer, state_dict, metadata,
            #          prefix=cls.base_model_prefix + ".encoder.layers.5.")
            #     load(model_to_load.base_model.topic_layer, state_dict, metadata,
            #          prefix=cls.base_model_prefix + ".encoder.layers.5.")
            # else:
            #     load(model_to_load, state_dict, metadata, prefix=start_prefix)

                # load(model_to_load.base_model.decoder2,Decoder_state_dict,decoder_metadata)
                # load(model_to_load.base_model.encoder.emo_attn,final_layer_state_dict,final_layer_metadata)
                # inject_keys.append()
            # model_to_load.lm_head_first.weights=state_dict["model.lm_head.weights"]
            # model_to_load.lm_head_first.bias = state_dict["model.lm_head.bias"]
            # model_to_load.lm_head_second.weights = state_dict["model.lm_head.weights"]
            # model_to_load.lm_head_second.bias = state_dict["model.lm_head.bias"]
            # for i,item in enumerate(missing_keys):
            #     name=re.split(r"(decoder2.)",item)
            #     if len(name)>1:
            #         if Decoder_state_dict[name[-1]] !=None:
            #             del(missing_keys[i])
            #         else:
            #             continue
        ...
        #         print(re.split(r"(decoder2)",item))
        #         _,key,name=re.split(r"(decoder2)",item)
        #         model_keyname="model.decoder"+name
        #         model[item]=state_dict[model_keyname]
            # torch.nn.Parameter(torch.from_numpy(model_kwargs['decoder']).type(torch.FloatTensor))
        # model.base_model.decoder2.weight = torch.nn.Parameter(
        #     torch.from_numpy(model_kwargs['decoder']).type(torch.FloatTensor))
