...
from modeling.activations import get_activation
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    def tie_weights(self):
        ...
        #     convert.weight = nn.Parameter(w.clone())
            # convert[1].weight=nn.Parameter(w.clone())
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        if from_tf:
            ...
            if config.is_train==True:
                # load(model_to_load, state_dict, metadata, prefix=start_prefix) #in stage two
                load(model_to_load.base_model.shared.embed_token, state_dict, metadata, prefix=cls.base_model_prefix + ".shared.")
                load(model_to_load.base_model.encoder.embed_tokens.embed_token, state_dict, metadata,
                     prefix=cls.base_model_prefix + ".shared.")
                load(model_to_load.base_model.decoder1.embed_tokens.embed_token, state_dict, metadata,
                     prefix=cls.base_model_prefix + ".shared.")
                load(model_to_load.base_model.decoder2.embed_tokens.embed_token, state_dict, metadata,
                     prefix=cls.base_model_prefix + ".shared.")
                load(model_to_load.integration.embed_tokens.embed_token, state_dict, metadata,
                     prefix=cls.base_model_prefix + ".shared.")
                ...
                load(model_to_load.integration, state_dict, metadata,
                    prefix=cls.base_model_prefix + ".decoder.")
        ...
        if config.is_encoder_decoder==True and config.is_train == True:
            model.tie_weights()
