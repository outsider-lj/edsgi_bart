...
class BartEncoder(BartPretrainedModel):
    ...
    def forward(
        ...
        if encoder_token_type_ids is not None:
            encoder_token_type_embeds=self.embed_tokens(encoder_token_type_ids)

        embed_pos = self.embed_positions(input_shape)

        hidden_states = inputs_embeds + embed_pos+encoder_token_type_embeds
        hidden_states = self.layernorm_embedding(hidden_states)
        hidden_states = F.dropout(hidden_states, p=self.dropout, training=self.training)

        # expand attention_mask
        if vm_mask is not None:
            attention_mask= vm_mask
...
class BartModel(BartPretrainedModel):
    ...
    def set_num_special_tokens(self, special_tokens):
        ...
        # self.config.pad_token_id = self.config.vocab_size + self.config.n_special - 1
...
class MultiBartForConditionalGeneration(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [
        r"final_logits_bias",
        r"encoder\.version",
    ...
    def __init__(self, config: BartConfig):
        ...
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
        self.emo_head=nn.Linear(config.d_model, config.emo_class, bias=False)
        self.init_weights()
    ...
    def forward(
        ...
        lm_logits = self.lm_head(decoder_output) #+ self.final_logits_bias
        cls_state=encoder_outputs[:,0,:]
        emo_logits=self.emo_head(cls_state)
        masked_lm_loss = None
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            # TODO(SS): do we need to ignore pad tokens in labels?
        ...
        if not return_dict:
            output = (lm_logits,) + outputs[1:]
            return ((masked_lm_loss,) + output),emo_logits if masked_lm_loss is not None else output,emo_logits
        return lm_logits,emo_logits
        # return Seq2SeqLMOutput(
        #     loss=masked_lm_loss,
        #     logits=lm_logits,
